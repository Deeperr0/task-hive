...
// fmtDate not needed for input[type=date]; we'll normalize ourselves
const LazySideMenu = lazy(() => import("../../ui/SideMenu"));

// helper: always return YYYY-MM-DD or "" for <input type="date">
...
export default function EditTask({
	...
	useEffect(() => {
		...
		setDesc(currentTask.description ?? currentTask.notes ?? "");
	...
	const onSave = async () => {
		...
		const updates = {
			content: title.trim(),
			description: desc,
			notes: desc, // keep both in sync with your schema
			deadline, // keep yyyy-mm-dd; your addTask uses same format
			priority,
			status,
					...
					<div className="lg:col-span-2 space-y-6">
						...
						<div className="rounded-xl border p-4">
							...
							<textarea
								...
								className="w-full resize-y rounded-lg border px-3 py-2 text-sm outline-none focus:border-slate-400"
							/>
						</div>
						...
						<div className="rounded-xl border p-4">
							...
							<div className="relative">
								<input
									...
									className="w-full rounded-lg border px-3 py-2 pr-10 text-sm outline-none focus:border-slate-400"
								/>
